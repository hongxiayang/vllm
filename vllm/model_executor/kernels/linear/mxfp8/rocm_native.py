...
def _select_cfg(M, N, K):
    ...
    if K <= 1024:  # short-K (shared_down K=384/768; TP=8 o_proj K=1024)
        ...
        # large prefill: tall tile only for K>=1024 (K=384/768 prefer 128x128x128) and
        # when the grid fills the CUs; BLOCK_K=256 needs K%256==0.
        if M >= 4096 and K >= 1024 and K % 256 == 0 and occ >= 256:
            return 256, 128, 256, 8, 2
    ...
    # large prefill (M > 1024). qkv-class local N=1536 (TP=8 qkv / TP=4 shared_gate_up)
    # with deep K: a deeper BLOCK_K=512 at 128x128 beats the 256x128x256 tall tile here
    # (better K-reuse at this N, ~+5-7%). K%512==0 required (K=6144 ok).
    if 1280 < N <= 1536 and K % 512 == 0:
        return 128, 128, 512, 8, 2
    # otherwise the tall 256x128x256 tile wins on deep-K or very-large-M when it makes
    # enough workgroups; else the prior 128x128x256 tile (small local N under-occupies).
    if K % 256 == 0 and occ >= 128 and (K >= 4096 or M >= 4096):
        return 256, 128, 256, 8, 2
