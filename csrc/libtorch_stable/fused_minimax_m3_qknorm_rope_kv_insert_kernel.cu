...
//
// `kHasIndex`, `kProcessIndex`, and `kInsertKV` are compile-time template bools,
// so branch decisions that distinguish the dense layer from the sparse layer
...
template <typename scalar_t, typename cache_t, Fp8KVCacheDataType kv_dt,
          typename out_idx_t, bool kHasIndex, bool kInsertKV, bool kProcessIndex,
          bool kFp8Idx>
...
void launchFusedMiniMaxM3(
  ...
  #define LAUNCH(HAS_INDEX, INSERT, PROCESS_INDEX, FP8, OUT_T)              \
    fusedMiniMaxM3QNormRopeKVInsertKernel<                                  \
        ...
        nq, nkv, niq, block_size, kv_s_block, kv_s_kv, kv_s_token,          \
        kv_s_head)
  // clang-format on
#endif
...
#define CALL_FUSED_MINIMAX_M3(_RAW_T, CACHE_T, KV_DTYPE)                       \
  vllm::minimax_m3_fused_ops::launchFusedMiniMaxM3<st, CACHE_T, KV_DTYPE>(     \
      ...
      kv_s_head, has_index, insert_kv, process_index, fp8_idx, stream)
