...
main () {
  ...
  while [[ $# -gt 0 ]]; do
    case $1 in
      "-h"|"--help")
        python3 /app/vllm/benchmarks/benchmark_throughput.py --help
        return 0
        ;;
      "--dataset")
        dataset_path="$2"
  ...
  torchrun --standalone --nnodes=1 --nproc-per-node=2 --log_dir /dockerx/logdir -r 3 -t 3 /app/vllm/benchmarks/benchmark_throughput.py -tp 2 --dataset "$dataset_path" --model "$model_path" "${positional_args[@]}"
  return $?
