...
main () {
  ...
  while [[ $# -gt 0 ]]; do
    case $1 in
      "-h"|"--help")
        python3 /vllm-workspace/benchmarks/benchmark_throughput.py --help
        return 0
        ;;
      "--dataset")
        dataset_path="$2"
  ...
  do
      echo "gpu $i"
      CUDA_VISIBLE_DEVICES="$i" python3 /vllm-workspace/benchmarks/benchmark_throughput.py --dataset "$dataset_path" --model "$model_path" -tp 1 "${positional_args[@]}" &
  done
  ...
  #python3 /vllm-workspace/benchmarks/benchmark_throughput.py  --dataset "$dataset_path" --model "$model_path" "${positional_args[@]}"
  return $?
